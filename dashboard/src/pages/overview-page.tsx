...
export function OverviewPage() {
  const overviewQuery = useQuery({
    queryKey: ["overview"],
    queryFn: async () => {
      const [user, experiments, datasets, repositories] = await Promise.all([
        api.getMe(),
        api.listExperiments(),
        api.listDatasets(),
      ...
      return {
        user,
        experiments,
        datasetGroups: groupDatasets(datasets),
        repositories,
      };
    },
  });
  ...
  const { user, experiments, datasetGroups, repositories } = overviewQuery.data;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <StatCard label="Experiments" value={formatCount(experiments.length)} detail="Defined training groups currently visible to your account." />
        <StatCard label="Datasets" value={formatCount(datasetGroups.length)} detail="Logical datasets, each grouping one or more immutable versions." />
        <StatCard label="Repositories" value={formatCount(repositories.length)} detail="Model repositories that will hold versioned artifacts over time." />
      </div>
