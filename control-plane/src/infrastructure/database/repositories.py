...
class SqlAlchemyModelRepo(ModelRepo):
    ...
    async def _get_status_id(self, name: str) -> uuid.UUID:
        stmt = select(ModelStatusORM).where(ModelStatusORM.name == name)
        result = await self._session.execute(stmt)
        status = result.scalar_one_or_none()
        if status:
            return status.id

        new_status = ModelStatusORM(id=uuid.uuid4(), name=name)
...
class SqlAlchemyRunRepo(RunRepo):
    ...
    async def _get_status_id(self, name: str) -> uuid.UUID:
        stmt = select(RunStatusORM).where(RunStatusORM.name == name)
        result = await self._session.execute(stmt)
        status = result.scalar_one_or_none()
        if status:
            return status.id

        new_status = RunStatusORM(id=uuid.uuid4(), name=name)
        self._session.add(new_status)
        await self._session.flush()
        return new_status.id
...
class SqlAlchemyDatasetRepo(DatasetRepo):
    ...
    async def _get_status_id(self, name: str) -> uuid.UUID:
        ...
        stmt = select(DatasetStatusORM).where(DatasetStatusORM.name == name)
        result = await self._session.execute(stmt)
        status = result.scalar_one_or_none()
        if status:
            return status.id

        new_status = DatasetStatusORM(id=uuid.uuid4(), name=name)
        self._session.add(new_status)
        await self._session.flush()
        return new_status.id
